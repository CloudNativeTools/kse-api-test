pipeline {
    ...
    environment {
        image_name = 'kse_api_test'
    }
    stages {
        stage('check out') {
            steps {
                // Get some code from a GitHub repository
                git credentialsId: 'kse-ssh', url:'https://github.com/CloudNativeTools/kse-api-test', branch: '${branch}'
    ...
    post {
        always {
            ...
            mimeType: 'text/html',
            subject: "【KSE自动化巡检】${env.JOB_NAME} - Build # ${env.BUILD_NUMBER}: $currentBuild.result",
            body: """
             <!DOCTYPE html>
            <html>
            <head>
