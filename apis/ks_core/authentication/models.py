...
@define(kw_only=True)
class OauthSpec:
    token: str = field(metadata={"description": "access token"})


@define(kw_only=True)
class OauthStatus:
    authenticated: bool = field(metadata={"description": "is authenticated"})
    user: Optional[Dict] = field(default=None, metadata={"description": "user info"})


@define(kw_only=True)
class OauthTokenReview:
...
@define(kw_only=True)
class PkixAttributeTypeAndValue:
    Type: List[int] = field()
    Value: Any = field()
...
class PkixName:
    ...
    ExtraNames: List[PkixAttributeTypeAndValue] = field()
    Locality: List[str] = field()
    Names: List[PkixAttributeTypeAndValue] = field()
    Organization: List[str] = field()
    OrganizationalUnit: List[str] = field()
    PostalCode: List[str] = field()
...
class UrlUserinfo:
    password: str = field()
    passwordSet: bool = field()
    username: str = field()


@define(kw_only=True)
class UrlURL:
    ForceQuery: bool = field()
    Fragment: str = field()
    Host: str = field()
    OmitHost: bool = field()
    Opaque: str = field()
    Path: str = field()
    RawFragment: str = field()
    RawPath: str = field()
    RawQuery: str = field()
    Scheme: str = field()
    User: UrlUserinfo = field()


@define(kw_only=True)
class X509Certificate:
    AuthorityKeyId: str = field()
    BasicConstraintsValid: bool = field()
    CRLDistributionPoints: List[str] = field()
    DNSNames: List[str] = field()
    EmailAddresses: List[str] = field()
    ExcludedDNSDomains: List[str] = field()
    ExcludedEmailAddresses: List[str] = field()
    ExcludedIPRanges: List[NetIPNet] = field()
    ExcludedURIDomains: List[str] = field()
    ExtKeyUsage: List[int] = field()
    ...
    KeyUsage: int = field()
    MaxPathLen: int = field()
    MaxPathLenZero: bool = field()
    NotAfter: datetime = field()
    NotBefore: datetime = field()
    OCSPServer: List[str] = field()
    PermittedDNSDomains: List[str] = field()
    PermittedDNSDomainsCritical: bool = field()
    ...
    SubjectKeyId: str = field()
    URIs: List[UrlURL] = field()
    UnhandledCriticalExtensions: List[List[int]] = field()
    UnknownExtKeyUsage: List[List[int]] = field()
    Version: int = field()
...
class JoseJSONWebKeySet:
    keys: List[JoseJSONWebKey] = field()
