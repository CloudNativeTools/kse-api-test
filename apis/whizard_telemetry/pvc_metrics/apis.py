...
__ALL__ = ["HandlePVCMetricsQueryAPI"]


@define(kw_only=True)
@router.get("/kapis/monitoring.kubesphere.io/v1beta1/persistentvolumeclaim_metrics")
class HandlePVCMetricsQueryAPI(BaseAPI[MonitoringMetrics]):
    """None"""
    ...
    class QueryParams:
        ...
        storageclass: Optional[str] = field(
            default=None, metadata={"description": "Storageclass name."}
        )
        storageclass_resources_filter: Optional[str] = field(
            default=None,
            metadata={
                "description": "The StorageClass filter consists of a regexp pattern. It specifies which StorageClass data to return."
            },
        )
        pvc: Optional[str] = field(
            default=None, metadata={"description": "PersistentVolumeClaim name."}
        )
    ...
    endpoint_id: Optional[str] = field(default="handlePVCMetricsQuery")
