# -*- coding:utf-8 -*-
"""
whizard-events 租户侧权限测试
验证不同角色对事件查询 API 的访问权限
...
class TestTenantPermissionPlatformRole:
    ...
    def test_pl_admin_query_events_statistics(self):
        """pl-admin 可查询事件统计"""
    ...
    def test_pl_admin_query_events_histogram(self):
        """pl-admin 可查询事件直方图"""
        account = ACCOUNTS["pl_admin"]
        with UserContext(account["user"], account["pwd"]):
    ...
    def test_pl_admin_query_events(self):
        """pl-admin 可查询事件列表"""
        account = ACCOUNTS["pl_admin"]
        with UserContext(account["user"], account["pwd"]):
            res = query_events()
...
class TestTenantPermissionClusterRole:
    ...
    def test_cluster_admin_query_histogram(self):
        """cluster-admin 可查询事件直方图"""
        account = ACCOUNTS["cluster_admin"]
        with UserContext(account["user"], account["pwd"]):
            res = query_events_histogram()
            assert res.cached_response.raw_response.status_code == 200

    def test_cluster_admin_query_events(self):
        """cluster-admin 可查询事件列表"""
        account = ACCOUNTS["cluster_admin"]
        with UserContext(account["user"], account["pwd"]):
            res = query_events()
            assert res.cached_response.raw_response.status_code == 200
    ...
    def test_cluster_viewer_query_histogram(self):
        """cluster-viewer 可查询事件直方图"""
        account = ACCOUNTS["cluster_viewer"]
        with UserContext(account["user"], account["pwd"]):
            res = query_events_histogram()
            assert res.cached_response.raw_response.status_code == 200

    def test_cluster_viewer_query_events(self):
        """cluster-viewer 可查询事件列表"""
        account = ACCOUNTS["cluster_viewer"]
        with UserContext(account["user"], account["pwd"]):
            res = query_events()
            assert res.cached_response.raw_response.status_code == 200
...
class TestTenantPermissionWorkspaceRole:
    ...
    def test_ws_admin_query_events(self):
        """ws-admin 可查询事件列表"""
        account = ACCOUNTS["ws_admin"]
        with UserContext(account["user"], account["pwd"]):
            res = query_events()
    ...
    def test_ws_viewer_query_events(self):
        """ws-viewer 可查询事件列表"""
        account = ACCOUNTS["ws_viewer"]
        with UserContext(account["user"], account["pwd"]):
            res = query_events()
...
class TestTenantPermissionProjectRole:
    ...
    def test_pro_admin_query_events(self):
        """pro-admin 可查询事件列表"""
        account = ACCOUNTS["pro_admin"]
        with UserContext(account["user"], account["pwd"]):
            res = query_events()
    ...
    def test_pro_operator_query_histogram(self):
        """pro-operator 可查询事件直方图"""
        account = ACCOUNTS["pro_operator"]
        with UserContext(account["user"], account["pwd"]):
            res = query_events_histogram()
            assert res.cached_response.raw_response.status_code == 200

    def test_pro_operator_query_events(self):
        """pro-operator 可查询事件列表"""
        account = ACCOUNTS["pro_operator"]
        with UserContext(account["user"], account["pwd"]):
            res = query_events()
            assert res.cached_response.raw_response.status_code == 200
    ...
    def test_pro_viewer_query_events(self):
        """pro-viewer 可查询事件列表"""
        account = ACCOUNTS["pro_viewer"]
        with UserContext(account["user"], account["pwd"]):
            res = query_events()
