...
def resolve_log_time_range(data_key: str = "last_1h") -> dict:
    from utils.test_data_helper import load_test_data
    raw = load_test_data("whizard_telemetry", "log_query/logs", "time_ranges", default={}, replace_vars=False)
    tr = raw.get(data_key, {})
    if not tr:
...
def query_logs_statistics(time_range_key: str = "last_1h", cluster: str = "host", **extra_params):
    tr = resolve_log_time_range(time_range_key)
