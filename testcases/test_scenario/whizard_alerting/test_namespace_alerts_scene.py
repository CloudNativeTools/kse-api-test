...
class TestNamespaceAlertScene:
    ...
    def test_02_list_rule_groups(self, host_cluster, test_namespace):
        """
        步骤2: 查看规则组列表
        """
        set_current_cluster(host_cluster)
...
class TestNamespaceAlertSceneMember:
    ...
    def test_02_list_rule_groups(self, member_cluster, test_namespace_member):
        """步骤2: 查看规则组列表"""
        set_current_cluster(member_cluster)
        try:
            api = HandleListRuleGroupsAPI(
                path_params=HandleListRuleGroupsAPI.PathParams(cluster=member_cluster, namespace=test_namespace_member),
                enable_schema_validation=False,
                response=None
            )
            api.query_params.page = "1"
