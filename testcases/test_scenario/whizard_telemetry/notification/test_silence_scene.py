"""
静默策略场景测试 - 完整工作流
流程：创建 -> 编辑信息 -> 编辑时间 -> 编辑条件 -> 编辑YAML -> 查看 -> 删除
"""
import pytest
...
class TestSilenceScenario:
    ...
    def test_01_create_silence(self):
        """创建静默策略"""
    ...
    def test_03_patch_duration(self):
        """编辑静默时间（duration）"""
    ...
    def test_04_patch_matcher(self):
        """编辑静默条件（matcher）"""
    ...
    def test_06_get_detail(self):
        ...
        get_res = get_api.send()
        assert get_res.cached_response.raw_response.status_code == 200
        detail_data = get_res.cached_response.raw_response.json()
        assert detail_data.get("metadata", {}).get("name") == SILENCE_NAME
        logger.info("6. 静默策略查看成功")

    def test_07_delete_silence(self):
        """删除静默策略"""
